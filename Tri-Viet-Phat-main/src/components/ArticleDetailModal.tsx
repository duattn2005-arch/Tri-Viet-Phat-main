...
export const ArticleDetailModal: React.FC<ArticleDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div
        className="bg-white  max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative border border-[#e5e5e5]"
      ...
      >
        ...
        <button
          onClick={onClose}
          aria-label="Đóng"
          className="absolute top-4 right-4 w-9 h-9 rounded-full bg-white/90 hover:bg-[#e5e5e5] text-[#555555] flex items-center justify-center transition-colors z-10 "
        >
        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          {article.keyPoints && article.keyPoints.length > 0 && (
            <div className="p-4  bg-[#f2f2f2]/60 border border-[#e5e5e5] space-y-2">
              ...
              <ul className="space-y-1.5 text-[13.5px] text-[#111111]">
                {article.keyPoints.map((point, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    <span className="material-symbols-outlined text-[#111111] text-[16px] shrink-0 mt-0.5">
                      check
                    </span>
                    <span>{point}</span>
                  </li>
          ...
          <div className="pt-6 border-t border-[#e5e5e5] flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#f7f7f7] p-4 ">
            <div className="flex items-center gap-3">
              <img
                ...
                className="h-10 w-auto max-h-10 object-contain  bg-white border border-[#e5e5e5] p-0.5"
              />
              <div>
                <span className="text-[13px] font-bold text-[#111111] block">
